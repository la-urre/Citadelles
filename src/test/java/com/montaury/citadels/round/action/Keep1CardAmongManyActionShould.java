...
public class Keep1CardAmongManyActionShould {
    ...
    public void be_executable_if_pile_has_enough_cards() {
        boolean executable = action.canExecute(aPlayer(), roundAssociations(), pileWith2Cards);
    ...
    public void ask_the_player_to_choose_between_the_2_drawn_cards() {
        action.execute(associationBetween(aPlayer(controller), Character.KING), roundAssociations(), pileWith2Cards);
    ...
    public void keep_all_drawn_cards_when_player_has_library() {
        Player player = aPlayerWith(cityWith(Card.LIBRARY), controller);
